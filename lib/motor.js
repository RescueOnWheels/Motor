...
class Motor {
   ...
   */
  Backwards() {
    const backwards = [7, 3, 165, 1, 3, 165, 1];

    debug('Going backwards at %d:%d.', backwards[1], backwards[2]);
  ...
  Forward({ speed, direction, balance }) {
    ...
    const forward = [7, (leftSpeed >> 8) & 255, leftSpeed & 255, leftDir, (rightSpeed >> 8) & 255, rightSpeed & 255, rightDir]; // eslint-disable-line max-len

    debug('Going forwards at L(%d:%d) and R(%d:%d).', forward[1], forward[2], forward[4], forward[5]);
